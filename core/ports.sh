...
ask_custom_port() {
    ...
    while true; do
        echo -e "\n${C_BOLD}${C_CYAN}--- Configuración de Red: ${SERVICE_NAME} ---${C_RESET}"
        echo -e "  - Rango seguro recomendado: ${C_GREEN}49152 a 65535${C_RESET} (Puertos dinámicos/privados)"
        echo -e "  - Puerto sugerido por defecto: ${C_BOLD}${C_CYAN}${DEFAULT_PORT}${C_RESET}"
        ...
        if is_port_in_use "$CHOSEN_PORT" "$PROTO"; then
            log_error "El puerto ${CHOSEN_PORT}/${PROTO} ya está ocupado por otro servicio en este servidor."
            continue
        fi

        log_ok "Puerto ${CHOSEN_PORT}/${PROTO} disponible y verificado."
        ...
        if command -v ufw >/dev/null 2>&1; then
            ...
            log_ok "Regla UFW añadida para ${CHOSEN_PORT}/${PROTO}."
        fi
